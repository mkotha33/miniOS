...
public class op extends JFrame {
	...
	private JLabel lblNoOfTimes,lblPhilosopher1,lblPhilosopher2,lblPhilosopher3,lblPhilosopher4,lblPhilosopher5;
		...
		SwingWorker<Boolean, Integer> worker = new SwingWorker<Boolean, Integer>() {
			 ...
			 protected Boolean doInBackground() throws Exception {
				 ...
				 class dine implements Runnable{
						...
						{
							Thread.sleep(5000);
							System.out.println(Integer.toString(id1)+" is thnking");
							status="thinking";
						...
						public int pickup(int i) throws InterruptedException
						{
							...
							{
								for(int j=0;j<5;j++){
									System.out.println("problem        "+curr[j]+" 111person-"+which[j]);
								}
							}*/
							...
							if(which[i-1]==i && curr[i]!=1){
								curr[i]=1;
								which[i]=i;
								System.out.println("The person picked is"+Integer.toString(id1));
								/*for(int j=0;j<5;j++)
								{
									System.out.println(i+"  111stick_state-"+curr[j]+" 111person-"+which[j]);
								}*/
								a= 2;
							...
							if(i==0){
								if(curr[stick]!=1){
								      ...
								      System.out.println("                   entered  "+stick+"\n");
									}
									if(which[stick]==i && curr[i]!=1){
										...
										lblNewLabel[i].setVisible(false);
										System.out.println("The person picked is"+Integer.toString(i));
										for(int j=0;j<=stick;j++)
										{
											System.out.println(id1+"  111stick_state-"+curr[j]+" 111person-"+which[j]);
										}
										a= 2;
										status="Chopstick picked!!";
										canvas[i].setBackground(Color.YELLOW);
						...
						public void eat(int i) throws InterruptedException
						{
							...
							System.out.println(Integer.toString(id1)+" is eating");
							status="eating";
							for(int j=0;j<=stick;j++)
							{
								System.out.println(id1+" stick_state-"+curr[j]+" person-"+which[j]);
							}
							label[i].setText(status);
							canvas[i].setBackground(Color.GREEN);
							Thread.sleep(5000);
							ecount[i]++;
							lblPhilosopher[i].setText("Philosopher_"+Integer.toString(i+1)+" : "+Integer.toString(ecount[i]));
						}
						public void putdown(int i) throws InterruptedException
						{
							...
							System.out.println(Integer.toString(id1)+" dropped the chopsticks");
							label[i].setText(status);
							canvas[i].setBackground(Color.GRAY);
							for(int j=0;j<=stick;j++)
							{
								System.out.println("dropppp   "+id1+" stick_state-"+curr[j]+" person-"+which[j]);
							}
							if(i>0)
							{
								thinking(i-1);
			  ...
			  for (int i = 0; i <n; i++) {
			   ...
			   a[i].start();
			   System.out.println("kjasdbasdvadgfakhbfakghdfadvhdsfvhgdf    "+i+   a[i].status);
			   publish(i);
			  }
			  return true;
